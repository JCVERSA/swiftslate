...
# (CDN mirror: irm https://cdn.jsdelivr.net/gh/JCVERSA/swiftslate@main/desktop/install.ps1 | iex)
...
$hashes = @{
    ...
    "configure.ps1"    = "996D8EA01E872A4D1A836BA189DB65265A7F5FF53689FB3D86C5EE38646E204B"
...
try {
    ...
    # Runtime helpers are updated together with the app. Commands are only copied
    # on a fresh install so user customizations survive updates.
    foreach ($name in @("text_styles.py", "secure_config.py", "configure.ps1")) {
        Get-File $name (Join-Path $installDir $name)
    }
    ...
    if (-not (Test-Path $configPath)) {
        ...
        if ($prov -eq "2") {
            ...
            $cfg.api_keys = if ([string]::IsNullOrWhiteSpace($key)) { @() } else { @($key) }
            Write-Host ""
        ...
        if ([string]::IsNullOrWhiteSpace([string]$cfg.api_keys[0]) -and $cfg.provider -ne "custom") {
            $cfg.api_keys = @()
            Write-Host "  No API key. Offline styles and local commands will still work." -ForegroundColor Yellow
        }
        ...
        # Keep API keys out of config.json. CryptProtectData scopes each value to
        # the current Windows user profile and is also used by configure.ps1.
        $protectedKeys = @()
        foreach ($key in @($cfg.api_keys)) {
            $protected = Protect-ApiKey ([string]$key)
            if ($protected) { $protectedKeys += $protected }
        }
        $cfg.Remove("api_keys")
        $cfg.api_keys_protected = $protectedKeys
