...
https://github.com/JCVERSA/swiftslate/tree/main/desktop
"""
...
from secure_config import SecureConfigError, read_config
from text_styles import DEFINITIONS, style_for_command, transform
...
MF_POPUP = 0x00000010
MENU_ID_EXIT = 1002
MENU_ID_STYLE_BASE = 1100
STYLE_MENU_ITEMS = (
    (MENU_ID_STYLE_BASE + 0, "bold", "Bold"),
    (MENU_ID_STYLE_BASE + 1, "italic", "Italic"),
    (MENU_ID_STYLE_BASE + 2, "mono", "Monospace"),
    (MENU_ID_STYLE_BASE + 3, "bubble", "Bubble"),
    (MENU_ID_STYLE_BASE + 4, "gothic", "Gothic"),
    (MENU_ID_STYLE_BASE + 5, "smallcaps", "Small caps"),
    (MENU_ID_STYLE_BASE + 6, "normal", "Normal text"),
)
STYLE_BY_MENU_ID = {menu_id: command for menu_id, command, _ in STYLE_MENU_ITEMS}
...
def load_config():
    ...
    try:
        ...
        log(f"ERROR: protected config could not be opened: {e}")
        notify("SwiftSlate", "Could not decrypt API keys for this Windows user.", NIIF_ERROR)
        return False
    ...
    if config.get("_protected_key_errors"):
        log("WARNING: Some protected API keys could not be decrypted; offline commands remain available")
    if not parsed_keys:
        # Offline styles, clipboard helpers, and canned replacers must work
        # without an account or network. AI commands report the missing key
        # only when they are invoked.
        if "api_keys_protected" in config:
            log("No API keys configured; offline commands remain available")
        else:
            log("WARNING: No protected API keys configured; offline commands remain available")
    ...
    if os.path.exists(commands_path):
        try:
            ...
            for cmd in cmd_list[:MAX_COMMANDS]:
                ...
                if cmd_type not in ("ai", "replacer-text", "replacer-shell", "style"):
    ...
    # Essential local styles are built in as well as being present in
    # commands.json. This keeps an older user configuration from silently
    # losing the offline MVP styles after an update.
    for style_name, _, description in DEFINITIONS:
        if style_name not in new_commands:
            new_commands[style_name] = {
                "type": "style",
                "prompt": description,
                "value": style_name,
            }
...
def _clear_clipboard_for_capture():
    """Clear the clipboard and return its new sequence number."""
    owner = hwnd_main or None
    if not user32.OpenClipboard(owner):
        return None
    try:
...
def apply_style_to_selection(style_name, target_hwnd):
    """Apply a local style to the selection in the last focused Windows app.

    Windows has no universal equivalent of Android's PROCESS_TEXT menu. The
    tray submenu is the explicit desktop equivalent: select text, open the
    SwiftSlate tray menu, and choose a style. Clipboard exclusion formats are
    used throughout so this operation does not add the selected text to
    clipboard history or cloud sync.
    """
    global processing
    if processing:
        _notify_debounced("SwiftSlate is already processing text.", NIIF_INFO)
        return
    style = style_for_command(style_name)
    if style is None or not target_hwnd:
        _notify_debounced("Select text in another app first.", NIIF_WARNING)
        return
    ...
    prev_extra = _snapshot_nontext_clipboard()
    try:
        sequence = _clear_clipboard_for_capture()
        if sequence is None:
            _notify_debounced("Could not access the clipboard.", NIIF_ERROR)
            return
        if user32.GetForegroundWindow() != target_hwnd:
            user32.SetForegroundWindow(target_hwnd)
            time.sleep(key_delay)
        if not _wait_modifiers_released(1.0) or user32.GetForegroundWindow() != target_hwnd:
            _notify_debounced("The selected app is no longer focused.", NIIF_WARNING)
            return
        send_keys("^c")
        selected = None
        for _ in range(50):
            ...
            if user32.GetClipboardSequenceNumber() != sequence:
                ...
                break
        if selected is None:
            _notify_debounced("No text selection was captured.", NIIF_WARNING)
            return

        styled = transform(style, selected)
        if not set_clipboard_silent(styled):
            _notify_debounced("Could not prepare the styled text.", NIIF_ERROR)
            return
        time.sleep(key_delay)
        if user32.GetForegroundWindow() != target_hwnd:
            _notify_debounced("The selected app lost focus. The result is on the clipboard.", NIIF_WARNING)
            return
        send_keys("^v")
        _notify_debounced(f"Applied {style_name} style.", NIIF_INFO)
        time.sleep(0.2)
    except Exception as exc:
        log(f"Selection style failed: {exc}")
        _notify_debounced("Could not style the selected text.", NIIF_ERROR)
    finally:
        if prev_clip is not None or prev_extra:
            if user32.GetClipboardOwner() == hwnd_main:
                set_clipboard_silent(prev_clip or "", prev_extra)
...
def do_replacer(trigger_name, cmd_type, value):
    ...
    try:
        ...
        if cmd_type == "replacer-text":
            ...
            style = style_for_command(value)
            if style is None:
                shell_error = "unknown local style"
            else:
                # Unlike a normal text replacer, a style transforms the text before
                # the trigger instead of appending a canned value to it.
                replacement = transform(style, before)
        ...
        if replacement:
            if abort_event.is_set():
                ...
                # Style commands replace the pre-trigger text. Other replacers
                # append their configured value as before.
                output = replacement if cmd_type == "style" else before + replacement
                paste_text(output)
...
def handle_trigger(trigger_name):
    ...
    try:
        if trigger_name == "undo":
            ...
            if cmd["type"] in ("replacer-text", "replacer-shell", "style"):
    ...
    try:
        if msg == WM_INPUT:
            ...
            if lparam == WM_RBUTTONUP:
                ...
                # Save the last app that received input before the tray menu
                # takes focus. This preserves its text selection.
                selection_target = last_fg_hwnd or user32.GetForegroundWindow()
                user32.SetForegroundWindow(hwnd)
                hmenu = user32.CreatePopupMenu()
                if hmenu:
                    style_menu = user32.CreatePopupMenu()
                    if style_menu:
                        for menu_id, _, label in STYLE_MENU_ITEMS:
                            user32.AppendMenuW(style_menu, MF_STRING, menu_id, label)
                        user32.AppendMenuW(hmenu, MF_POPUP, style_menu, "Style selected text")
                    ...
                    if cmd in STYLE_BY_MENU_ID:
                        style_name = STYLE_BY_MENU_ID[cmd]
                        threading.Thread(
                            target=lambda: apply_style_to_selection(style_name, selection_target),
                            daemon=True,
                        ).start()
                    elif cmd == MENU_ID_EXIT:
