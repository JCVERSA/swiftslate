...
class GeminiClient {
    ...
    suspend fun validateKey(apiKey: String): Result<String> = withContext(Dispatchers.IO) {
        ...
        try {
            connection = ApiConnections.openHttps(
    ...
    suspend fun fetchModels(apiKey: String): Result<List<String>> = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            while (true) {
                ...
                val body: String
                try {
                    connection = ApiConnections.openHttps("$MODELS_URL?pageSize=1000$pageSuffix")
                    connection.requestMethod = "GET"
                    connection.setRequestProperty("x-goog-api-key", apiKey)
                    connection.connectTimeout = 15_000
                    connection.readTimeout = 15_000
    ...
    private fun doGenerate(
        ...
        return try {
            ...
            val jsonBody = JSONObject().apply {
                ...
                // No explicit safetySettings: thresholds and their defaults vary by model and
                // tier, so the provider default governs. This previously pinned BLOCK_NONE for
                // all five categories, which for HARM_CATEGORY_CIVIC_INTEGRITY loosened filtering
                // beyond the provider default on models where that default is stricter. Blocks
                // are already handled gracefully (finishReason SAFETY et al. plus the
                // promptFeedback.blockReason path below map to the localized safety message).
