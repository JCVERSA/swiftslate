...
internal object ApiClientUtils {
    ...
    /**
     * Removes one known key — the key just submitted for validation — from provider text.
     * [redactSecrets] only recognizes known key prefixes, so a Custom endpoint echoing an
     * unknown-format key ("Incorrect API key provided: ...") would otherwise be displayed
     * verbatim on the Keys screen. Keys shorter than [MIN_REDACT_KEY_LENGTH] are left alone:
     * replacing a 2-character substring would mangle ordinary prose, and a key that short
     * carries negligible entropy anyway.
     */
    fun redactSubmittedKey(text: String, key: String): String {
        if (key.length < MIN_REDACT_KEY_LENGTH) return text
        return text.replace(key, "***")
    }

    private const val MIN_REDACT_KEY_LENGTH = 8
