...
class OpenAICompatibleClient {
    ...
    suspend fun validateKey(apiKey: String, endpoint: String): Result<String> = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            connection = ApiConnections.openProviderUrl("$baseUrl/models")
    ...
    private fun probeModels(url: String, apiKey: String): Boolean {
        ...
        return try {
            probe = ApiConnections.openProviderUrl(url)
    ...
    private fun httpGet(url: String, apiKey: String?): Probe {
        var connection: HttpURLConnection? = null
        return try {
            connection = ApiConnections.openProviderUrl(url)
            connection.requestMethod = "GET"
    ...
    private fun doGenerate(
        ...
        return try {
            val baseUrl = endpoint.trimEnd('/')
            connection = ApiConnections.openProviderUrl("$baseUrl/chat/completions")
